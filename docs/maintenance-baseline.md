...
| `UseKtx` | 低 | 133 | 119 | 14 | 0 | 0 | 119 | PR 3 与 PR 4g 已安全消除 7 项；PR 4h 将 7 个只读像素、布局方向、子项非空及可见性 getter 转为同一 API 的 KTX 访问器；其余继续逐类审查 |
| `UselessParent` | 中 | 1 | 1 | 0 | 0 | 1 | 0 | PR 4f（#59）审查确认需移动漫画菜单边距、背景和测量职责，缺少稳定页面截图，本轮精确延期 |
| `VectorPath` | 中 | 1 | 1 | 0 | 0 | 0 | 1 | 图形精度与渲染风险 |
| **合计** |  | **881** | **809** | **67** | **5** | **43** | **766** | 逐 ID 审查中；PR 3 与 PR 4a 至 PR 4h 已完成 72 项修复/抑制，并精确延期 43 项 |
