...
| `UseKtx` | 低 | 133 | 126 | 7 | 0 | 0 | 126 | PR 3 的显式 Intent 构造安全消除 1 项；PR 4g 将 `RecyclerAdapter` 的 6 处 `SparseArray.size()` 转为同一 API 的 KTX `size` 属性；其余继续逐类审查 |
| `UselessParent` | 中 | 1 | 1 | 0 | 0 | 1 | 0 | PR 4f（#59）审查确认需移动漫画菜单边距、背景和测量职责，缺少稳定页面截图，本轮精确延期 |
| `VectorPath` | 中 | 1 | 1 | 0 | 0 | 0 | 1 | 图形精度与渲染风险 |
| **合计** |  | **881** | **816** | **60** | **5** | **43** | **773** | 逐 ID 审查中；PR 3 与 PR 4a 至 PR 4g 已完成 65 项修复/抑制，并精确延期 43 项 |
