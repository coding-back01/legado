...
从最新受保护分支实施的替代依赖变更 MUST 在自身全部必需检查成功并进入 `master` 后，重新读取被替代机器人 Pull Request 的编号、状态、head ref、head SHA 和依赖范围，只关闭仍明确匹配批准清单且内容已被替代的对象。系统 MUST 在闭环记录中关联替代变更，并仅按重新核对的精确 ref/SHA 删除对应远端分支；若对象已被 Dependabot 自动关闭或删除分支，系统 MUST 核对原范围后补充替代记录，不得重新打开对象；若编号或 ref 被复用为新的依赖范围，系统 MUST 将其作为新对象保留。不得使用分支名前缀、glob 或过期快照扩大清理范围。
...
- **THEN** 系统比较原批准范围与当前依赖范围，停止对当前对象的关闭和分支删除
- **AND** 当前对象代表新依赖范围时必须保留，未漂移对象也只能继续执行各自已经满足的精确操作

#### Scenario: 机器人自动关闭原批准范围
- **WHEN** Dependabot 在替代变更合并后自动关闭原 Pull Request 并删除其分支，且原编号、ref、SHA 和依赖范围仍可核对
- **THEN** 系统在已关闭对象上补充关联替代 Pull Request 和 merge SHA 的记录
- **AND** 不重新打开对象或重建已删除分支
