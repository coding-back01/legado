...
- 未推广实现快照：实验分支 `codex/add-kindle-page-turn-animation` 的提交 `5ef423314`；最终归档状态由远端不可变标签 `archive/kindle-page-turn-animation-no-go-20260813` 保留。该快照只用于保存软件候选、状态机、测试和复现上下文，不得整体合入主分支，不得作为功能完成或发布依据。
