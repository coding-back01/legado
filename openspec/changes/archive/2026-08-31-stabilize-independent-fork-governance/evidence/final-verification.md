...
- schema：`spec-driven`
- 核验分支：`codex/release-verification-evidence-v2`
- 核验基线提交：`ed09c78b42e67503fdfcde555fc6e92ee978da1a`
- 发布提交：`cef2fbb2dbdb6771686b04c68447a6f5caea964e`
- 远端复核时间：2026-08-31 01:48 +0800
- 规范规模：5 个 capability、29 个 Requirement、78 个 Scenario
- 阶段证据：29 份版本化 Markdown；真机与模拟器原始文件只保存在 Git 忽略且权限受限的本地目录

生产实现、持续门禁、普通版发布和发布后更新器回验已经通过。29 个 Requirement 和
78 个 Scenario 均已建立实现、测试、负向合同或过程证据锚点；其中 28 个 Requirement
已经达到终态，`总变更只能在发布闭环后归档` 正按设计处于“已发布、尚待归档与精确分支
清理”的中间状态。

本报告完成并勾选任务 9.1–9.3 后，任务进度为 87/91。9.4–9.7 是必须按顺序完成的
归档、归档 PR 门禁、合并后复核和精确分支清理任务；它们不是生产实现缺陷，但在全部完成前
不得宣称一次性治理 91/91 完成。
...
| 维度 | 结果 | 结论 |
|---|---|---|
| 完整性 | 87/91 tasks；29/29 Requirement 有证据；78/78 Scenario 有处理锚点 | 生产、发布和归档前核验完整；4 个归档闭环任务按时序待完成 |
| 正确性 | 28/29 Requirement 已达终态；1/29 正在执行规定的归档时序 | 未发现规范缺失或实现偏离；可以开始归档 PR，不能提前宣称总变更完成 |
...
| 项目 | 2026-08-31 读回结果 |
|---|---|
| 远端 `master` | `cef2fbb2dbdb6771686b04c68447a6f5caea964e` |
...
| Release 身份 | workflow、tag、Release target 与远端 `master` 均等于发布提交 |
...
| 远端分支 | 35 条；其中总变更的已合并短期分支等待任务 9.5 生成精确 ref/SHA 白名单后再清理 |
...
实际失败处置均有代码、测试、CI、远端、模拟器或真机证据；`进行中`仅用于规范明确要求在
归档后才能达到终态的最后一个 Requirement。
...
| 总变更只能在发布闭环后归档 | 发布完成但尚未归档；任一完成条件缺失 | 当前正处于第一个 Scenario：新普通版已公开且更新器已回验，活跃变更仍未归档；任务 9.4–9.7 明确阻止在归档、合并后复核和分支清理缺失时宣称完成 | `tasks.md`、本报告 | 进行中，符合预期时序 |
...
3. #42、#44、#45、#73 是新月度策略的普通 Dependabot PR，不属于遗留队列，不在本变更中
   强行关闭或合并。
4. 当前远端短期治理分支尚未执行最终删除。任务 9.5 必须从已合并 PR 实时读取精确 head
   ref、head SHA 和 merge 状态生成白名单；任务 9.7 只删除 SHA 未漂移且不再使用的精确对象，
   禁止使用 `codex/*` 或其他 glob。
...
- 9.4：创建 `codex/archive-fork-governance`，归档变更并同步主规范；归档 PR 只能包含规范、
  证据和必要文档。
- 9.5：归档 PR 检查全绿后，记录所有治理 PR 的精确 head ref/head SHA/merge 状态并生成
  删除白名单。
- 9.6：合并归档 PR 后同步本地 `master`，重新执行严格 OpenSpec、空白检查及远端总复核。
- 9.7：按白名单和 SHA lease 精确删除已合并短期分支，保留 PR、Release、CI artifact、
  失败草稿和 OpenSpec 归档，并提交最终一次性治理报告。

这些任务必须完成后才能把总变更标记为 91/91；当前可以开始 9.4，但不能直接宣称完成或
先行批量删分支。
...
结果：本报告与任务 9.1–9.3 写入后复跑，`openspec validate --all --strict` 为
3 passed、0 failed；`git diff --check` 无输出。CLI 读回进度为 87/91，剩余任务精确为
9.4–9.7。

## 最终判断

没有发现生产实现、发布结果与五份 delta spec、设计决策之间的未记录偏离。质量、安全、
分发、模拟器、指定真机和公开 Release 门禁已经满足；中危与 warning 债务均有明确判断。

结论：**可以开始 OpenSpec 归档 PR，但总变更尚未完成。** 必须依次完成 9.4–9.7，并在
归档合并后的 `master` 重新核验后，才可给出一次性治理完成结论。
